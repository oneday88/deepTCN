...
class TCN(nn.Block):
    def __init__(self, dilation_depth=2, n_repeat=5, **kwargs):
        ...
        #elf.post_res= nn.Sequential()
        self.TCN= nn.Sequential()
        with self.name_scope():
            ## The embedding part
            self.id_embedding=nn.Embedding(1046,10)
            self.nYear_embedding = nn.Embedding(3,2)
            self.nMonth_embedding = nn.Embedding(12,3)
            for d in self.dilations:
                self.TCN.add(ResidualTCN2(d=d))
            self.post_res=futureResidual(xDim=6)
            ...
            self.Q10 = nn.Dense(1, flatten=False)
            self.Q50 = nn.Dense(1, flatten=False)
            self.Q90 = nn.Dense(1, flatten=False)    
