...
def switchActivation(actType='relu'):
    assert actType in set([ 'tanh', 'relu','pRelu','leakyRelu', 'swish'])
    if actType in ['relu','tanh']:
            act1 = nn.Activation(actType)
    elif actType=='pRelu':
        act1 = nn.PReLU()
    elif actType=='leakyRelu':
        act1 = nn.LeakyReLU(alpha=0.01)
    else:
        act1 = nn.swish()
    return act1
...
class TCN(nn.Block):
    def __init__(self,inputSize=168, outputSize=24 ,dilations=[1,2,4,8,16,20,32],nResidue=35, actType='relu' ,dropout=0.2, **kwargs):
        ...
        with self.name_scope():
            ...
            self.stationEmbedding = nn.Embedding(370,10)
            self.nYearEmbedding = nn.Embedding(3,2)
            self.nMonthEmbedding = nn.Embedding(12,3)
            self.mDayEmbedding = nn.Embedding(365,5)
    ...
    # embed the auxiliary variables
        ...
        embedTrain = embedConcat[:,0:self.inputSize,:10] # only consider the id for the input series
