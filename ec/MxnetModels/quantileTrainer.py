...
class nnTrainer(object):
    ...
    def probEvaluator(self, nnModel, testX, testX2, testY, lossFunc,mode='Normal'):
        ...
        if(mode =='logTransform'):
            validPredQ10 = np.exp(validPredQ10)-1
            validPredQ50 = np.exp(validPredQ50)-1
            validPredQ90 = np.exp(validPredQ90)-1
    ...
    def fit(self, mark,  trainX, futureTrainX, trainY, testX, futureTestX, testY, paramsDict):
        ...
        for e in tqdm(range(epochs), desc='epochs'):
            ...
            if not testX is None:
                testLoss, rho50, rho90 = self.probEvaluator(self.model, testX, futureTestX, testY, lossFunc, mark)
