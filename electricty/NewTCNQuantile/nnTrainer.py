...
def nn_trainer(train_mark, model, train_data, test_conv_X, test_data_X,test_data_Y, trainer_params_list, ctx):
    ...
    for epoch in trange(epoches):
        ...
        NDMean, ND0, ND1,ND2,ND3,ND4,ND5,ND6 = group_ND(valid_pred, valid_true,370)
        NRMSEMean, NRMSE0, NRMSE1,NRMSE2,NRMSE3,NRMSE4,NRMSE5,NRMSE6 = group_NRMSE(valid_pred, valid_true,370)
        risk50Mean, risk500, risk501,risk502,risk503,risk504,risk505,risk506 = group_rho_risk(valid_pred, valid_true,370,0.5)
        risk90Mean, risk900, risk901,risk902,risk903,risk904,risk905,risk906 = group_rho_risk(valid_pred, valid_true,370,0.9)
        print(" average ND: %f, valid NRMSE %s" % (NDMean, str(ND0)+str(ND1)+"-"+str(ND2)+'-'+'-'+str(ND3)+'-'+str(ND4)+'-'+str(ND5)+'-'+str(ND6)))
        print(" average NRMSE: %f, valid NRMSE %s" % (NRMSEMean, str(NRMSE0)+str(NRMSE1)+"-"+str(NRMSE2)+'-'+'-'+str(NRMSE3)+'-'+str(NRMSE4)+'-'+str(NRMSE5)+'-'+str(NRMSE6)))
        print(" average risk50: %f, valid risk50 %s" % (risk50Mean, str(risk500)+str(risk501)+"-"+str(risk502)+'-'+'-'+str(risk503)+'-'+str(risk504)+'-'+str(risk505)+'-'+str(risk506)))
        print(" average risk90: %f, valid risk90 %s" % (risk90Mean, str(risk900)+str(risk901)+"-"+str(risk902)+'-'+'-'+str(risk903)+'-'+str(risk904)+'-'+str(risk905)+'-'+str(risk906)))
